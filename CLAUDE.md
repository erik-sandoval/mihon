...
- Remove temporary debug logging once the root cause is confirmed and fixed — don't leave it in the codebase. But don't remove it preemptively either: keep it in place through a build/push cycle until the user has actually confirmed on-device that the fix worked. A fix that looks right from the logs alone isn't confirmed — if it turns out wrong, you'll want that same instrumentation for the next round instead of re-adding it from scratch.
